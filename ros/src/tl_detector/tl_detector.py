...
class TLDetector(object):
    ...
    def image_cb(self, msg):
        ...
        full_path = os.path.join("saved_images_2", "{}.png".format(str(uuid.uuid4())))
        ...
        #Processing every x-th image
            ...
            if self.state != state:
                ...
                # DEBUG
                #rospy.logerr(">> Model light :%s",self.state)
                ...
                if state == TrafficLight.YELLOW or state == TrafficLight.RED or self.load_status == False:
                	light_wp = light_wp
                elif state == TrafficLight.GREEN and self.light_waypoint_id_temp  and self.closest_id_temp and ( self.light_waypoint_id_temp  - self.closest_id_temp < 20) and ( self.light_waypoint_id_temp  - self.closest_id_temp >=(-5)):
                	light_wp = -3
                elif self.light_waypoint_id_temp  and self.closest_id_temp and ( self.light_waypoint_id_temp  - self.closest_id_temp < TL_DIST) and ( self.light_waypoint_id_temp  - self.closest_id_temp >3):
                	light_wp = -2
                else:
                	light_wp = -1
    ...
    def process_traffic_lights(self):
        ...
        if(self.pose and self.waypoints):    
            ...
            #rospy.logerr(">> car_wp_id :%s",car_wp_id)

            #rospy.logerr(">> intheloop :%s",self.intheloop)
        ...
        #rospy.logerr(">> self.light_waypoint_id_temp :%s",self.light_waypoint_id_temp)
        #rospy.logerr(">> self.closest_id_temp :%s",self.closest_id_temp)
        #rospy.logerr(">> self.closest_light_temp :%s",self.closest_light_temp)
        ...
        if closest_light and self.light_waypoint_id_temp and self.closest_id_temp  and ( self.light_waypoint_id_temp  - self.closest_id_temp < TL_DIST):
            ...
            # For debugging purposes only
            rospy.logerr(">> Model light :%s",state)
